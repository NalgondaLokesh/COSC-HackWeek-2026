# Kavach - Secret Scanner 🛡️

A lightweight, powerful, and minimalistic security tool that runs entirely locally. Kavach scans your code right before you commit, and uses pattern matching (Regex) to ensure you never accidentally push API keys, passwords, or tokens to GitHub.
...
- **Web Interface:** Modern web dashboard with real-time monitoring capabilities.
...
To use the web interface with real-time monitoring:
```bash
pip install flask
python app.py
```
...
5. Add a fake secret to it (e.g. `const awsKey = "AKIA_FAKE_1234567890";`)
6. Try to commit the file:
   ```bash
   git add config.js
   git commit -m "Add config file"
   ```
7. Watch the terminal light up in **RED** as Kavach aborts the commit and warns you about the leaked AWS Key! This makes for a perfect screen recording.
